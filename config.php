...
function verifyCsrf(): void {
    ...
    // Reset token after use (one-time token pattern)
...
function sanitizeFloat($val, float $min = -90.0, float $max = 90.0): float {
...
function secureImageUpload(array $file, string $dir, int $maxSize = 2097152): ?string {
    ...
    if (!in_array($realMime, $allowedMime)) return null;
    ...
    if ($dims === false) return null;
    if ($dims[0] > 4096 || $dims[1] > 4096) return null;
